...
def validarUsuario(usuario, clave) :
   global sesion
   if (usuario == usuarioEnBCP and clave == claveEnBCP) :
     sesion = True
     return True
   return False
def login() :
...
def ejecutar() :
   ...
   opcion = int(input("1 => Depositar, 2 => Retirar y 3 => Consultar saldo: "))
...
while (iniciar == True) :
   ejecutar()
   respuesta = input("¿Deseas realizar otra operación? (SI => s, y NO => valor diferete de s): ")
   if (respuesta == "s" or respuesta == "si" or respuesta == "S" or respuesta == "SI") :
     iniciar = True
   else:
     iniciar = False
     print("Gracias por utilizar los servicios del Banco de Credito del Peru...")
